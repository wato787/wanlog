...
| 犬プロフィール | 名前・犬種・誕生日・アイコンを登録 |

### 2-2. MVP以降に追加する機能候補

| 機能 | 備考 |
|---|---|
| いいね機能 | likesテーブルは設計済み、実装のみ後回し |
| アルバム画面 | グリッド表示で写真を振り返る |
...
#### 2. 犬プロフィール作成
- 入力項目: 名前（必須）・犬種・誕生日・アイコン画像
- 初回ログイン時のみ表示（グループ未所属の場合）
- 「作成する」ボタン → グループ・犬を同時に作成
...
| 犬タグづけ | MVPでは不要。後付けで追加（post_dogsテーブル追加予定） |
